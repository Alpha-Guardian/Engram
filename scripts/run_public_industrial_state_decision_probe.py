...
def _build_no_trunk_checkpoint(current_checkpoint: Path, out_path: Path) -> Path:
    if out_path.exists():
        return out_path
    payload = _read_json(current_checkpoint)
    for key in [
        "logiqa_trunk_recurrent_blocks",
        "logiqa_trunk_recurrent_merge_audit",
        "support_trunk_recurrent_source",
    ]:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the public industrial state-decision second-task probe.")
    ...
    out_json = (
        ...
        else (repo_root / "results/experiments/industrial_state_decision_probe.json")
    )

    taskset = repo_root / "results/experiments/industrial_state_decision_logiqa_lite_v1.txt"
    out_dir = workspace_root / "artifacts/reports/public_cross_task"
    checkpoints_dir = workspace_root / "artifacts/reports/public_cross_task/checkpoints"
    ...
    no_trunk_checkpoint = checkpoints_dir / "current_scientific_surface_no_trunk_industrial_state_decision_probe.json"
    ...
    no_trunk_checkpoint = _build_no_trunk_checkpoint(current_checkpoint, no_trunk_checkpoint)
    ...
    for surface_id, label, checkpoint, output_tag in surfaces:
        ...
        reports[surface_id] = report_path
        rows.append(
            {
                "surface_id": surface_id,
                "label": label,
                "industrial_state_decision_accuracy": round(_accuracy_from_report(report_path), 6),
                "checkpoint": {
    ...
    current_acc = float(row_map["current_scientific_surface"]["industrial_state_decision_accuracy"])
    parent_acc = float(row_map["frozen_authoritative_parent"]["industrial_state_decision_accuracy"])
    no_trunk_acc = float(row_map["current_no_trunk_ablation"]["industrial_state_decision_accuracy"])
    trained_linear_acc = float(row_map["parent_gpu_trained_linear_head_baseline"]["industrial_state_decision_accuracy"])

    bundle = {
        "bundle_kind": "public_industrial_state_decision_probe_v2",
        "generated_at": _now(),
        "task": "industrial_state_decision_logiqa_lite_v1",
        "metric": "accuracy",
        "taskset": {
            "path": str(taskset.relative_to(repo_root)).replace("\\", "/"),
            "sha256": _sha256(taskset),
            "samples": len(parsed_samples),
            "task_style": "light_domain_shifted_logiqa_style_mcq",
        ...
        "pairwise": pairwise,
        "notes": [
            "This is a lightly domain-shifted second-task probe that wraps current-win reasoning shapes in industrial and embedded review language while preserving a LogiQA-style multiple-choice reasoning surface.",
            "It is intended to test whether the current promoted surface can retain a positive same-parent signal outside the literal public LogiQA benchmark.",
            "It is still an exploratory transfer probe, not a claim of broad industrial generalization or a replacement for a full second-task promotion line.",
        ],
    }
