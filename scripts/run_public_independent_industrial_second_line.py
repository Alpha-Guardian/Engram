...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the public independent industrial second-line benchmark.")
    ...
    out_json = (
        ...
        else (repo_root / "results/experiments/independent_industrial_second_line.json")
    )

    taskset = repo_root / "results/experiments/industrial_state_decision_logiqa_v2.txt"
    ...
    independent_checkpoint = workspace_root / "artifacts/reports/public_cross_task/industrial_wrapper_pass8_materialized_from_slice.json"
    compromise_checkpoint = workspace_root / "artifacts/reports/public_cross_task/subsets/industrial_wrapper_targets_5_6_8_10.json"
    parent_preserving_checkpoint = workspace_root / "artifacts/reports/public_cross_task/subsets/industrial_wrapper_targets_5_8_10_16.json"
    ...
    model_cache = {
        ...
        "independent": load_model(independent_checkpoint),
        "compromise": load_model(compromise_checkpoint),
        "parent_preserving": load_model(parent_preserving_checkpoint),
    }
    pairwise = {
        "independent_vs_parent": _pair_counts(parsed_samples, model_cache["parent"], model_cache["independent"], predict_logiqa_option),
        "independent_vs_no_trunk": _pair_counts(parsed_samples, model_cache["no_trunk"], model_cache["independent"], predict_logiqa_option),
        "independent_vs_trained_linear": _pair_counts(parsed_samples, model_cache["trained_linear"], model_cache["independent"], predict_logiqa_option),
        "independent_vs_current": _pair_counts(parsed_samples, model_cache["current"], model_cache["independent"], predict_logiqa_option),
    }

    row_map = {row["surface_id"]: row for row in rows}
    independent_acc = float(row_map["independent_industrial_second_line"]["accuracy"])
    ...
    bundle = {
        "bundle_kind": "public_independent_industrial_second_line_v1",
        "generated_at": _now(),
        "task": "industrial_state_decision_logiqa_v2",
        "metric": "accuracy",
        "taskset": {
            "path": str(taskset.relative_to(repo_root)).replace("\\", "/"),
            "sha256": _sha256(taskset),
            "samples": len(parsed_samples),
            "task_style": "industrial_or_embedded_reasoning_mcq",
            "scope": "independent_second_task_capability_line",
        },
        "rows": rows,
        "summary": {
            "independent_vs_parent_delta": round(independent_acc - parent_acc, 6),
            "independent_vs_no_trunk_delta": round(independent_acc - no_trunk_acc, 6),
            "independent_vs_trained_linear_delta": round(independent_acc - trained_linear_acc, 6),
            "independent_vs_current_delta": round(independent_acc - current_acc, 6),
            "independent_above_parent": bool(independent_acc > parent_acc),
            "independent_above_no_trunk": bool(independent_acc > no_trunk_acc),
            "independent_above_trained_linear": bool(independent_acc > trained_linear_acc),
            "independent_above_current_surface": bool(independent_acc > current_acc),
        },
        "pairwise": pairwise,
        "notes": [
            "This bundle is the public proof that the same frozen-parent trunk pipeline can instantiate a new independent second-task capability line, not just improve the current LogiQA surface.",
            "The parent-derived independent industrial line is evaluated on a separate small industrial-state decision benchmark rather than on the public official LogiQA benchmark.",
            "Because this benchmark is still small, it should be read as a second-task capability-production proof, not as proof of broad industrial generalization.",
        ],
    }
